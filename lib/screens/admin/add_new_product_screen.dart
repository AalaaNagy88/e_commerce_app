...
class _AddNewProductScreenState extends State<AddNewProductScreen> {
  ...
  Widget controlsBuilder(BuildContext context, {onStepCancel, onStepContinue}) {
      onStepContinue()async{
        print(_currentStep);
        if(_currentStep==0&&Provider.of<ImagePickerProvider>(context,listen: false).image != null){
          _isLoading(true);
          _imageUrl = await uploadImage(context);
          _isLoading(false);
          setState(() => _currentStep += 1);
        }
        if(Provider.of<ImagePickerProvider>(context,listen: false).image == null){
          showDialog(context: context,builder: (_)=>AlertDialog(content:Text("Please, pick product image"),actions: [  TextButton(
              child: Text("Okay"),onPressed: ()=>Navigator.pop(context)),],));
        }
        _catogery = CustomDropdownList.category;
        if(_currentStep==1){
          if (ProductDetialsStep.globalKey.currentState.validate() && _catogery != null) {
        ...
        if(_currentStep==2){
          try {
            _isLoading(true);
            uploadProductInfo(ProductDetialsStep.name,ProductDetialsStep.price,ProductDetialsStep.description,_catogery,_imageUrl);
            _isLoading(false);
